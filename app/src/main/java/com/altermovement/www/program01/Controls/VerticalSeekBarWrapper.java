...
public class VerticalSeekBarWrapper extends FrameLayout {
    ...
    private void onSizeChangedTraditionalRotation(int w, int h, int oldw, int oldh) {
        final VerticalSeek seekBar = getChildSeekBar();
    ...
    private void applyViewRotation(int w, int h) {
        ...
        if (seekBar != null) {
            ...
            switch (rotationAngle) {
                case VerticalSeek.ROTATION_ANGLE_CW_90:
                    ...
                    break;
                case VerticalSeek.ROTATION_ANGLE_CW_270:
    ...
    private VerticalSeek getChildSeekBar() {
        final View child = (getChildCount() > 0) ? getChildAt(0) : null;
        return (child instanceof VerticalSeek) ? (VerticalSeek) child : null;
    }
